...
export default function ResetPassword() {
  ...
  return <main className="auth-page"><Container><Card className="auth-card border-0 mx-auto" style={{maxWidth: 620}}><Card.Body className="p-4 p-md-5"><h1 className="h2">Choose a new password</h1>
    ...
    {!message && <Form onSubmit={(event) => void submit(event)}><Form.Group className="mb-3"><Form.Label htmlFor="reset-password">New password</Form.Label><PasswordInput id="reset-password" value={password} onChange={setPassword} autoComplete="new-password" minLength={8} /></Form.Group><Form.Group className="mb-3"><Form.Label htmlFor="reset-confirm">Confirm password</Form.Label><PasswordInput id="reset-confirm" value={confirm} onChange={setConfirm} autoComplete="new-password" minLength={8} /></Form.Group><Button type="submit" className="w-100" disabled={busy}>{busy ? 'Resetting…' : 'Reset password'}</Button></Form>}
